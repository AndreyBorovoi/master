...
export class TokenWithModelIDGuard implements CanActivate {
  constructor(private userService: UserService, private aiserviceService: AiserviceService) {}

  async canActivate(context: ExecutionContext): Promise<boolean> {
    const request = context.switchToHttp().getRequest();
    const user = await this.userService.getUserByToken(request.params.token);
    const service = await this.aiserviceService.getAIServiceByModelId(request.params.modelId)
